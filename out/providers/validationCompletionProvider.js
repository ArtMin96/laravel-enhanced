...
exports.LaravelValidationCompletionProvider = void 0;
const vscode = require("vscode");
const fs = require("fs");
const path = require("path");
class LaravelValidationCompletionProvider {
    constructor(workspaceRoot) {
        this.workspaceRoot = workspaceRoot;
        this.validationRules = [];
        this.databaseTables = new Map();
        this.modelColumns = new Map();
    ...
    parseMigrationForSchema(filePath) {
        try {
    ...
    isInValidationContext(beforeCursor, document, position) {
    ...
    provideValidationRuleCompletions(beforeCursor, document, position) {
        const completions = [];
        ...
        const availableRules = this.validationRules.filter(rule => !currentRules.includes(rule.name) && this.isRuleApplicable(rule, currentRules, fieldName));
    ...
    extractCurrentRules(beforeCursor, isArrayContext) {
        if (isArrayContext) {
    ...
    extractFieldName(beforeCursor) {
        const fieldMatch = beforeCursor.match(/['"`]([^'"`]+)['"`]\s*=>/);
        return fieldMatch ? fieldMatch[1] : '';
    }
    isRuleApplicable(rule, currentRules, fieldName) {
        ...
        if (conflicts.some((conflict) => currentRules.includes(conflict))) {
            return false;
        }
    ...
    createRuleCompletion(rule, isArrayContext, fieldName) {
    ...
    createRuleDocumentation(rule, fieldName) {
    ...
    createRuleInsertText(rule, isArrayContext, fieldName) {
    ...
    getParameterPlaceholder(param, fieldName, index) {
    ...
    getRuleSortText(rule, fieldName) {
        ...
        if (fieldName) {
            if (fieldName.includes('email') && rule.name === 'email')
                priority = 1;
            if (fieldName.includes('password') && rule.name === 'confirmed')
                priority = 2;
            if (fieldName.endsWith('_id') && (rule.name === 'exists' || rule.name === 'integer'))
                priority = 2;
        }
...
exports.LaravelValidationCompletionProvider = LaravelValidationCompletionProvider;
//# sourceMappingURL=validationCompletionProvider.js.map