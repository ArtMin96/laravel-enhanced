...
exports.RequestFieldCommands = exports.RequestFieldCodeActionProvider = exports.EnhancedModelCompletionProvider = exports.LaravelRequestCompletionProvider = void 0;
const vscode = require("vscode");
const fs = require("fs");
const path = require("path");
class LaravelRequestCompletionProvider {
    ...
    parseValidationRules(rulesContent) {
        const fields = [];
    ...
    extractArrayRules(arrayContent) {
        const rules = [];
        const rulePattern = /['"`]([^'"`]+)['"`]/g;
        let match;
    ...
    provideRequestFieldCompletion(beforeCursor, document, position) {
        const patterns = [
            // Model::create([
            /(\w+)::create\s*\(\s*\[\s*$/,
            // $model->update([
    ...
    inferRequestTypeFromMethod(document, position) {
    ...
    getRequestFieldCompletions(requestType) {
        ...
        request.fields.forEach(field => {
            ...
            item.documentation = new vscode.MarkdownString(`**Field:** ${field.field}\n\n` +
        ...
        if (request.fields.length > 0) {
            const addAllItem = new vscode.CompletionItem(`🔧 Add ALL Request Fields (${request.fields.length} fields)`, vscode.CompletionItemKind.Snippet);
            addAllItem.detail = `Insert all ${request.fields.length} fields from ${requestType}`;
            addAllItem.documentation = new vscode.MarkdownString(`**Fields to be added:**\n${request.fields.map(f => `- ${f.field}`).join('\n')}`);
    ...
    addAllRequestFields(editor, requestType) {
        const request = this.requests.get(requestType);
        ...
        vscode.window.showInformationMessage(`Added ${request.fields.length} fields from ${requestType}`);
    }
    getIndentation(document, position) {
...
exports.LaravelRequestCompletionProvider = LaravelRequestCompletionProvider;
// Enhanced Model Completion Provider with Request Integration
class EnhancedModelCompletionProvider {
    constructor(modelProvider, // LaravelIntelligentCompletionProvider
    requestProvider) {
    ...
    provideCompletionItems(document, position, token, context) {
        ...
        const requestCompletions = this.requestProvider.provideCompletionItems(document, position, token, context);
        if (requestCompletions.length > 0) {
            return requestCompletions;
        }
        // Then try model attribute completion
        if (this.modelProvider) {
            return this.modelProvider.provideCompletionItems(document, position, token, context);
        }
        return [];
    }
}
exports.EnhancedModelCompletionProvider = EnhancedModelCompletionProvider;
// Code Actions for Request Fields
class RequestFieldCodeActionProvider {
    ...
    provideCodeActions(document, range, context, token) {
        ...
        if (isInModelContext) {
            ...
            if (requestType) {
                ...
                if (request && request.fields.length > 0) {
                    const action = new vscode.CodeAction(`Add all ${request.fields.length} fields from ${requestType}`, vscode.CodeActionKind.Refactor);
                    action.command = {
    ...
    inferRequestType(document, position) {
        const text = document.getText();
        const lines = text.split('\n');
...
exports.RequestFieldCodeActionProvider = RequestFieldCodeActionProvider;
// Commands for Request Field Management
class RequestFieldCommands {
    static register(context, requestProvider) {
        // Add all request fields command
        context.subscriptions.push(vscode.commands.registerCommand('laravel.addAllRequestFields', (requestType) => {
        ...
        context.subscriptions.push(vscode.commands.registerCommand('laravel.showRequestFields', async () => {
        ...
        context.subscriptions.push(vscode.commands.registerCommand('laravel.generateRequestFromModel', async () => {
    ...
    static showRequestFieldsPanel(request) {
        const panel = vscode.window.createWebviewPanel('requestFields', `${request.name} Fields`, vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = this.generateRequestFieldsHtml(request);
    }
    static generateRequestFieldsHtml(request) {
    ...
    static async generateRequestFromModel(modelName) {
        // This would generate a FormRequest class based on model attributes
        vscode.window.showInformationMessage(`Feature coming soon: Generate ${modelName}Request from ${modelName} model`);
    }
}
exports.RequestFieldCommands = RequestFieldCommands;
//# sourceMappingURL=requestCompletionProvider.js.map