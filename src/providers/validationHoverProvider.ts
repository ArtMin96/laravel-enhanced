...
import { LaravelValidationCompletionProvider, ValidationRule } from './validationCompletionProvider';

export class ValidationHoverProvider implements vscode.HoverProvider {
    constructor(private validationProvider: LaravelValidationCompletionProvider) {}
    ...
    private isValidationRule(text: string, line: string): boolean {
    ...
    private createRuleHoverContent(rule: ValidationRule, fullText: string): vscode.MarkdownString {
    ...
    private extractRuleParameters(ruleText: string): string[] {
...
export class ValidationDiagnosticsProvider {
    ...
    private isValidationFile(document: vscode.TextDocument): boolean {
            ...
            validationRules.forEach(({ rule, range }) => {
                ...
                issues.forEach(issue => {
                    const diagnostic = new vscode.Diagnostic(
                        new vscode.Range(lineIndex, range.start, lineIndex, range.end),
                        issue.message,
                        issue.severity
                    );
    ...
    private extractValidationRulesFromLine(line: string): Array<{rule: string, range: {start: number, end: number}}> {
        const rules: Array<{rule: string, range: {start: number, end: number}}> = [];
    ...
    private isValidationRule(rule: string): boolean {
    ...
    private validateRule(rule: string, document: vscode.TextDocument, lineIndex: number): Array<{
        message: string,
...
export class ValidationCodeActionProvider implements vscode.CodeActionProvider {
    ...
    private createUnknownRuleActions(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
        const actions: vscode.CodeAction[] = [];
        ...
        if (match) {
            ...
            const suggestions = allRules.filter(rule => 
                this.calculateSimilarity(rule.name, unknownRule) > 0.6
            ).slice(0, 3);
    ...
    private createMissingParameterActions(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
        const actions: vscode.CodeAction[] = [];
    ...
    private createTableNotFoundActions(document: vscode.TextDocument, diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
        const actions: vscode.CodeAction[] = [];
    ...
    private createValidationImprovementActions(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] {
        const actions: vscode.CodeAction[] = [];
    ...
    private isValidationContext(lineText: string): boolean {
...
export class ValidationCommands {
    static register(context: vscode.ExtensionContext, validationProvider: LaravelValidationCompletionProvider) {
        // Add validation parameters command
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.addValidationParameters', 
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.convertValidationFormat',
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.sortValidationRules',
                (uri: vscode.Uri, lineNumber: number) => {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.showValidationRules', () => {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.generateValidationFromDB', async () => {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.validateAllRequests', () => {
                this.validateAllRequestFiles();
            })
        );
    }

    private static async addValidationParameters(
        uri: vscode.Uri, 
        range: vscode.Range, 
        validationProvider: LaravelValidationCompletionProvider
    ) {
        ...
        if (!rule || !rule.parameters) return;
    ...
    private static async convertValidationFormat(uri: vscode.Uri, lineNumber: number) {
        const document = await vscode.workspace.openTextDocument(uri);
    ...
    private static async sortValidationRules(uri: vscode.Uri, lineNumber: number) {
        const document = await vscode.workspace.openTextDocument(uri);
        const line = document.lineAt(lineNumber);
        const lineText = line.text;
    ...
    private static showValidationRulesPanel(validationProvider: LaravelValidationCompletionProvider) {
    ...
    private static generateValidationRulesHtml(rules: ValidationRule[]): string {
    ...
    private static async generateValidationFromDatabase(validationProvider: LaravelValidationCompletionProvider) {
    ...
    private static generateValidationRulesFromTable(table: any): string {
        const rules = table.columns.map((column: string) => {
...
${rules}
    ...
    private static validateAllRequestFiles() {
