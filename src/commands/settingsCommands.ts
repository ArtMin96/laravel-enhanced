...
export class SettingsCommands {
    static register(context: vscode.ExtensionContext) {
        context.subscriptions.push(
            vscode.commands.registerCommand('laravel.openSettings', () => {
                this.showSettingsPanel();
            })
        );
    }

    private static showSettingsPanel() {
        const panel = vscode.window.createWebviewPanel(
            'laravelSettings',
            'Laravel Enhanced Settings',
    ...
    private static updateSetting(key: string, value: any) {
        const config = vscode.workspace.getConfiguration('laravelEnhanced');
        config.update(key, value, vscode.ConfigurationTarget.Global);
    }

    private static resetAllSettings() {
        const config = vscode.workspace.getConfiguration('laravelEnhanced');
        const keys = [
    ...
    private static generateSettingsHtml(): string {
        const config = vscode.workspace.getConfiguration('laravelEnhanced');
